...
// ユニットは既定ですべて閉じ、「いま取り組んでいる1ユニット」だけを開く。
// 手動の開閉はメモリだけで保持し、リロードすると既定に戻す
// （マイペースの週間タブ weekOpen と同方式。localStorageには保存しない）
const unitOpen = new Map();

// 「いま取り組んでいるユニット」は最後に開いたユニットで判定する。
// カリキュラム順から外れて学ぶ（例: CSSの途中でRubyを先取り）場合も追従できるよう、
// 順番からの推測ではなく実際の操作履歴を1件だけ保存する
...
// 記録がない（初回・別ブラウザ・リセット後）ときの受け皿。
// カリキュラム順で最初の未完了ユニットを「取り組み中」とみなす
function firstIncompleteUnit() {
  const unit = UNITS.find(function (u) {
    return !u.locked && LESSONS.some(function (l) { return l.unit === u.id; }) && !unitComplete(u.id);
  });
  return unit ? unit.id : null;
}

function activeUnitId() {
  if (lastUnit && UNITS.some(function (u) { return u.id === lastUnit; })) return lastUnit;
...
// 見出しのカウントは力試しも1件として数える。
// レッスンだけで数えると、力試しが残っていても「3/3」となり終わった感が出るため
function unitCounts(unitId) {
  const unitLessons = LESSONS.filter(function (l) { return l.unit === unitId; });
  const rv = reviewForUnit(unitId);
  let done = unitLessons.filter(function (l) { return lessonProgress(l.id).done; }).length;
  let total = unitLessons.length;
  if (rv) {
    total += 1;
    if (reviewDone(rv.id)) done += 1;
  }
  ...
  UNITS.forEach(function (unit) {
    ...
    if (unit.locked) html += '<span class="locked-badge">🔒 ' + unit.note + "</span>";
      ...
      html += '<span class="unit-count' + (complete ? " complete" : "") + '">' +
        (complete ? "✔ " : "") + counts.done + "/" + counts.total + "</span>";
    }
    html += "</summary>";
...
function handleRestoreInput(text) {
  ...
  document.getElementById("restore-apply").addEventListener("click", function () {
    ...
    clearLastUnit();  // 復元後は、復元した進捗に応じたユニットが開くようにする
    currentLessonId = null;
    currentReviewId = null;
    document.getElementById("lesson-view").hidden = true;
    document.getElementById("review-view").hidden = true;
